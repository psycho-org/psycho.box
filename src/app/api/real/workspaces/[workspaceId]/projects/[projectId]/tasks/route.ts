...
export async function POST(
  ...
  if (!accountId) {
    console.error('[API] POST /workspaces/:workspaceId/projects/:projectId/tasks missing account id', {
      userId,
      accountFromQuery,
      hasToken: !!token,
    });
    return Response.json({ message: 'User ID required for project task create' }, { status: 400 });
  }
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    console.error('[API] POST /workspaces/:workspaceId/projects/:projectId/tasks failed', {
      workspaceId,
      projectId,
      accountId,
      status: res.status,
